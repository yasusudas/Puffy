...
3. **Firebase の ID / シークレットのペア** を再確認（シークレットだけ正しくても ID が違うと失敗します）:
   - Azure → **概要** の **アプリケーション (クライアント) ID** をコピー
   - Azure → **証明書とシークレット** の **値** 列をコピー（**シークレット ID** の GUID 列ではない）
   - Firebase Console → Microsoft で **両方** を貼り直して保存
   - ブラウザの開発者ツール（F12）→ Console の `[Puffy auth]` ログに `app 'xxxxxxxx-...'` と出たら、その ID が Firebase のアプリケーション ID と一致するか確認
...
| 「シークレットが正しくない」と出るがコピペは合っている | **アプリケーション ID** がシークレットと同じ Azure 登録か確認。古いアプリの ID のまま新シークレットだけ貼っていることが多い。Firebase で Microsoft をオフ→保存→オン→ID とシークレットを両方貼り直し |
| `AADSTS7000215` | シークレットの値が誤り、期限切れ、または ID とペアが不一致 |
| `AADSTS700016` | Firebase のアプリケーション ID が Azure に存在しない（別アプリの ID） |
| `AADSTS7000222` | シークレットの有効期限切れ。Azure で新規作成して Firebase に貼り直し |
